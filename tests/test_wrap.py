...
async def test_enforce_anthropic_allow_passes_through(opts: WardenOptions) -> None:
    ...
    client = warden_wrap(_anthropic_client(make_anthropic_message_with_tool_use()), opts)
...
async def test_enforce_openai_deny_raises(opts: WardenOptions) -> None:
    ...
    client = warden_wrap(_openai_client(make_openai_completion_with_tool_call(name="rm_rf")), opts)
...
async def test_text_only_response_bypasses_inspect() -> None:
    ...
    client = warden_wrap(_anthropic_client(text_only), WardenOptions(endpoint=FAKE_ENDPOINT))
    await client.messages.create(model="claude-x")
