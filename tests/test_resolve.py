"""`ClavenarPending.resolve()` end-to-end: catch the pending, drive the
...
async def test_resolve_returns_when_operator_approves() -> None:
    ...
    client = clavenar_wrap(_anthropic(make_anthropic_message_with_tool_use()), opts)
...
@respx.mock
async def test_resolve_raises_clavenar_denied_when_operator_denies() -> None:
    respx.post(f"{FAKE_ENDPOINT}/mcp").mock(return_value=httpx.Response(202, json=_pending_body()))
    respx.get(f"{FAKE_ENDPOINT}/pending/corr-001").mock(
    ...
    with pytest.raises(ClavenarPending) as exc:
        await client.messages.create(model="claude-x")
    with pytest.raises(ClavenarDenied) as denied:
        await exc.value.resolve(poll_interval_s=0.001, timeout_s=2.0)
...
async def test_resolve_propagates_terminal_404() -> None:
    ...
    with pytest.raises(ClavenarPending) as exc:
        await client.messages.create(model="claude-x")
    with pytest.raises(ClavenarTransportError) as terr:
        await exc.value.resolve(poll_interval_s=0.001, timeout_s=2.0)
