# warden-ai (Python) sequence diagrams

Five sequence diagrams covering the wire-level paths the Python SDK
can take: `warden_wrap` boot with the structural client + sync/async
...
`WardenPending.resolve` polling until decided, and the standalone
...
sibling at [`warden-ai-sdk`](https://github.com/vanteguardlabs/warden-ai-sdk).
...
## 1. `warden_wrap` — validate, detect, monkey-patch the right `.create`

`warden_wrap` runs once per client. It validates the option bag,
...
sequenceDiagram
    ...
    participant Wrap as warden_wrap
    ...
    alt any check fails
        V-->>Caller: raise WardenConfigError
    end
    Wrap->>D: _detect_client(client)
    alt client is None
        D-->>Caller: raise WardenConfigError
    end
    alt client.messages.create exists and callable
        ...
        D-->>Caller: raise WardenConfigError — client must expose messages.create or chat.completions.create
    end
...
parallel denies always produce the same `WardenDenied.tool_name`
...
sequenceDiagram
    ...
    par asyncio.gather one(call) for call in calls
        ...
        T->>T: headers — Content-Type, Authorization Bearer (if token), opts.extra_headers (X-Warden-Demo-Prefix etc)
        ...
        alt 200
            ...
            alt enforce mode (Sec 6 flowchart)
                ...
                Insp->>Insp: catch WardenTransportError as result
            end
        end
    ...
    loop results in submission order calls[i]
        opt opts.on_verdict
            Insp->>Caller: await on_verdict(verdict, WardenVerdictContext(tool_name, tool_use_id, tool_input))
        end
        alt result is WardenTransportError (observe only)
            ...
            Insp-->>Caller: raise WardenDenied(tool_name, reasons, review_reasons, intent_category, correlation_id)
        else enforce AND verdict.kind == pending
            Insp-->>Caller: raise WardenPending(tool_name, correlation_id, review_reasons, poll_once closure)
    ...
    loop every upstream chunk
        ...
        loop every choice_idx queued for inspection (BEFORE yielding chunk)
            ...
            par asyncio.gather per call
                ...
                L-->>T: 200 / 403 / 202 / 5xx
                T-->>Batch: WardenVerdict OR raise
            end
...
When enforce mode raises `WardenPending`, the partner catches it,
...
sequenceDiagram
    ...
    Note over Partner: Sec 2 or Sec 3 raised WardenPending — partner caught it
    ...
    loop while time.monotonic() < deadline
        ...
        alt 200
            ...
            alt view.decision == allow
                ...
                Pending-->>Partner: raise WardenDenied(intent_category='PendingDenied', correlation_id, ...)
            ...
            L-->>Hx: status
            Hx-->>T: response
            T-->>Poll: raise WardenTransportError(status)
            Poll-->>Pending: WardenTransportError with status 401 or 404
...
for `warden_wrap` to intercept. The partner drains the WS event
...
the raw string on parse failure so warden can still inspect the
...
sequenceDiagram
    ...
    T->>L: POST /mcp (same envelope and retry as Sec 2)
    L-->>T: 200 / 403 / 202 / 5xx
    T-->>Helper: WardenVerdict (no raise on deny — caller decides)
...
pass-through, `WardenDenied`, `WardenPending`, `WardenTransportError`,
or a `WardenConfigError` raised before any of those.

```mermaid
flowchart TD
    Start([wrapped.create call]) --> Cfg{validate_options AND<br/>_detect_client OK?}
    Cfg -->|no| Cerr[raise WardenConfigError<br/>partner bug — bad endpoint, missing client, sync callback on async wrap, etc.]
    ...
    Stream1 --> Loop1
    Sync2 --> Loop2[_inspect_all_sync — serial for-loop<br/>callbacks must be sync OR raise WardenConfigError]
    Stream2 --> Loop2
    ...
    Tport -->|enforce| Te[raise WardenTransportError — fail-closed]
    ...
    Vk -->|deny| Dn[raise WardenDenied — tool_name, reasons, review_reasons, intent_category, correlation_id]
    Vk -->|pending| Pn[raise WardenPending — correlation_id, review_reasons, async closure poll_once]
    ...
    Rd -->|deny within deadline| Rdn[raise WardenDenied — intent_category PendingDenied + decider_note]
    Rto[raise WardenTransportError — not decided within timeout_s]
    Rd -->|deadline trip| Rto
    Rd -->|401 or 404 terminal| Rterm[raise WardenTransportError immediately]
